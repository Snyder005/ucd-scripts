...
from ccs.power_control import PowerControl
...
class UCDPower(object):
    ...
    def __init__(self):
        ...
        # Define controls
        self.reb_controls = {'Analog' : PowerControl('Analog', self.bk9130b1, 1, 8.0),
                             'Heater' : PowerControl('Heater', self.bk9130b1, 2, 12.0),
                             'Digital' : PowerControl('Digital', self.bk9130b1, 3, 5.0),
                             'ClockLow' : PowerControl('ClockLow', self.bk9130b2, 1, 15.0),
                             'ClockHigh' : PowerControl('ClockHigh', self.bk9130b2, 2, 15.0),
                             'OD' : PowerControl('OD', self.bk1697b, 0, 38.0)}

        self.hvbias_control = PowerControl('HVBias', self.bk9184, 0, 50.0, current=0.001)
        self.otm_control = PowerControl('OTM', self.bk9130b2, 3, 5.0)
    ...
    def power_on(self):
        ...
        try:
            if not self.is_power_on():
                ...
                self.reb_controls['ClkHigh'].power_on()
                self.reb_controls['ClkLow'].power_on()
    ...
    def power_off(self):
        ...
        try:
            ...
            self.reb_controls['ClkLow'].power_off()
            self.reb_controls['ClkHigh'].power_off()
            sleep(20)
    ...
    def publish_state(self):
        ...
        # REB control
        for control in self.reb_control:
            name, state, voltage, _ = control.get_state()
            print('{0}: State = {1}, voltage = {2:.3f}'.format(name, state, voltage))

        # HVBias control
        name, state, voltage, _ = hvbias_control.get_state()
        print('{0}: State = {1}, voltage = {2:.3f}'.format(name, state, voltage))

        # OTM control
        name, state, voltage, _ = otm_control.get_state()
        print('{0}: State = {1}, voltage = {2:.3f}'.format(name, state, voltage))
