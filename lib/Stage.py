...
import time, fcntl, serial, socket, struct
import USBaddresses
...
class Stage(object):
    def __init__(self, master):
        ...
        self.stage_device_name=USBaddresses.stageaddress
    ...
    def zero_encoders(self):
        ...
        for i in range(3):
            self.fd_channel[i].write(b'\x00\x00\x00') # Write zeros to 24 bit PR register
            fcntl.ioctl(self.fd_channel[i], self.LOAD_CMD_REG, self.TRAN_PR_CNTR) # Transfers the PR register to the counter
        read_pos=self.read_encoders(self)
        return read_pos
    ...
    def read_encoders(self):
        ...
        while max_diff > 1:
            ...
            for i in range(3):
                ...
                max_diff = abs(last_read_pos - self.read_pos[i])
            return read_pos
    ...
    def move_stage(self,[x,y,z]):
        ...
        for i in range(3):
            ...
            time.sleep(0.5)
        time.sleep(0.5)
        new_pos=self.read_encoders(self)
        return new_pos
